	...
	public boolean deleteDiskFile(int deleteFileId) {
		...
		try {
			...
			if (rs.next()) {
				...
				attachFileNameDel = rs.getString("ATTACHFILENAMEDISK") == null ? "" : rs
						.getString("ATTACHFILENAMEDISK").trim();
	...
	if (fileId != 0 && !fileName.equalsIgnoreCase("") && officeFileItem != null) {
		if (deleteDiskFile(fileId))//删除数据库中fileid对应的服务器文件
		{
			if (attachFileItem != null)//如果上传附件为空,应该使用数据库中原有的附件数据
			{
				...
				rs.next();
				attachFileNameDisk = rs.getString("ATTACHFILENAMEDISK") == null ? "" : rs.getString(
						"ATTACHFILENAMEDISK").toString();
				rs.close();
			}
			mySqlStr = "update "
					+ officeFileInfoTableName
					+ " set filename=?,filesize=?,otherdata=?,filetype=?,filenamedisk=?,attachfilenamedisk=?,attachfiledescribe=? where id="
					+ fileId;
