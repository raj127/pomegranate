...
String templateFileUrl="/template/001/";//新建文档模板url
String otherData="";
...
if(isNewFile){
	...
	if(filetype.equalsIgnoreCase("excel")){	
		...
		templateFileUrl=templateFileUrl+"01-01.docx";
	}
...
<form id="form1" action="upLoadOfficeFile.jsp"
	enctype="multipart/form-data" style="padding: 0px; margin: 0px;">
<div id="editmain" class="editmain">
<div id="edittop" class="top"><img alt="重庆软航科技有限公司示例程序"
	src="images/edit_banner.jpg" /></div>
<div id="editmain_top" class="editmain_top">
<div id="edit_button_div" class="edit_button_div"><img
	alt="保存office文档" src="images/edit_save_office.gif"
	onclick="saveFileToUrl();" /> <img alt="保存html文档"
	src="images/edit_save_html.gif" onclick="saveFileAsHtmlToUrl();" /> <img
	alt="保存PDF" src="images/edit_save_pdf.gif"
	onclick="saveFileAsPdfToUrl();" /> <img alt="示例程序帮助文档"
	src="images/demohelp.jpg" onclick="NtkoHelp();" /></div>
<table border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td><img src="images/edit_main_top.jpg" alt="文件列表上框" /></td>
	</tr>
	<tr>
		<td class="edittablebackground"><!--示例标题--><b>ntko
		office文档控件示例-ms office文档编辑</b></td>
	</tr>
</table>
</div>
...
<ul class="ul">
	...
	<li
		onclick="OFFICE_CONTROL_OBJ.IsShowInsertMenu=!OFFICE_CONTROL_OBJ.IsShowInsertMenu;">"插入"菜单切换</li>
	<li
		onclick="OFFICE_CONTROL_OBJ.IsShowEditMenu=!OFFICE_CONTROL_OBJ.IsShowEditMenu;">"编辑"菜单切换</li>
	<li
		onclick="OFFICE_CONTROL_OBJ.IsShowToolMenu=!OFFICE_CONTROL_OBJ.IsShowToolMenu;">"工具"菜单切换</li>
</ul>
</div>
<div class="funbutton">
<ul class="ul">
...
<ul class="ul">
	...
	<li class="listtile"><select id="SignFileUrl"
		onchange="var signUrl=document.all('SignFileUrl').options[document.all('SignFileUrl').selectedIndex].value;if(signUrl==''){}else  addServerSign(signUrl);">
		<option value="""selected">请选择服务器印章</option>

	</select></li>
	<li class="listtile"><select id="picFileUrl"
		onchange="var picURL=document.all('picFileUrl').options[document.all('picFileUrl').selectedIndex].value;if(picURL==''){}else addPicFromUrl(picURL);">
...
<ul class="ul">
	...
	<%
                    ...
                    {
                    %>
	<li><select id="redHeadTemplateFile"
		onchange="var headFileURL=document.all('redHeadTemplateFile').options[document.all('redHeadTemplateFile').selectedIndex].value;if(headFileURL==''){};else insertRedHeadFromUrl(headFileURL);">
	...
	<li><select id="templateFile"
		onchange="var templateFileUrl=document.all('templateFile').options[document.all('templateFile').selectedIndex].value;if(templateFileUrl==''){};else openTemplateFileFromUrl(templateFileUrl);">
...
<table>
	...
	<tr>
		...
		<td width="20%"><input name="fileId" id="fileId" readOnly
			type="text" value="<%=fileId%>" /></td>
		<td width="8%">文件名称:</td>
		<td width="40%"><input name="filename" id="filename" type="text"
			value="<%=fileName%>" /></td>
		<td>&nbsp;</td>
	</tr>
	<tr>
		...
		<td><input id="otherData" name="otherData" align="left"
			type="text" value="<%=otherData%>" /></td>
		<td>上传文件:</td>
		<td><input class=fileup name="attachFile" type=file
			id="attachFile" />&nbsp;(选择附件)<br><input
			name="attachFileDescribe" type=text id=attachFileDescribe
			value="<%=attachFileDescribe%>">&nbsp;(附件说明)
		</td>
		<td>&nbsp;</td>
	</tr>
	<tr>
		<td>文档附件:</td>
		<td colspan="4"><!--附件列表--> <%if(!isNewFile&&!attachFileUrl.equalsIgnoreCase(""))
														{
...
<div id="esptoolbar" class="esptoolbar"><span class="listtile">安全电子印章系统功能:</span>
...
<div id="officecontrol"><script type="text/javascript"
	src="officecontrol/ntkoofficecontrol.js"></script>
<div id=statusBar
	style="height: 20px; width: 100%; background-color: #c0c0c0; font-size: 12px;"></div>
<script language="JScript" for=TANGER_OCX event="OnDocumentClosed()">
									setFileOpenedOrClosed(false);
								</script> <script language="JScript" for="TANGER_OCX"
...
<div id="edit_bottom" class="edit_bottom"><img alt=""
	src="images/edit_main_nether.jpg" />
<div id="conmpanyinfo" class="conmpanyinfo"><img alt="重庆软航科技有限公司"
	src="images/Companyinfo.jpg" />
