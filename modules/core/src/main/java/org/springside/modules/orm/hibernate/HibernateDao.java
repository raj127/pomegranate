		...
		if (page.isOrderBySetted()) {
			...
			for (int i = 0; i < orderByArray.length; i++) {
				if (Page.ASC.equals(orderArray[i])) {
					if (orderByArray[i].startsWith("convert")) {
						//从字符串convert(name, 'gbk')中提取name
						orderByArray[i] = orderByArray[i].substring(8, orderByArray[i].indexOf(","));
					}
					c.addOrder(GBKOrder.asc(orderByArray[i]));
				} else {
					if (orderByArray[i].startsWith("convert")) {
						//从字符串convert(name, 'gbk')中提取name
						orderByArray[i] = orderByArray[i].substring(8, orderByArray[i].indexOf(","));
					}
					c.addOrder(GBKOrder.desc(orderByArray[i]));
				}
			}
		}
		return c;
